...
def main():
    ...
    # Prepare dummy inputs (must match the model's forward signature)
    batch_size = 1
    dummy_image = torch.randn(batch_size, 3, args.image_size, args.image_size)
    dummy_caption = ['this is a dummy caption'] * batch_size
    dummy_cls_labels = torch.zeros(batch_size, 18, dtype=torch.long)  # Adjust class count if needed
    clip_k = 21
    clip_dim = 512
    ...
    # Patch forward if needed for thop
    ...
    model.forward = orig_forward  # Restore original
